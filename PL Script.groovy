node {
    ...
    stage('同步代码') {
        ...
        checkout([$class: 'GitSCM', branches: [[name: 'refs/heads/$branchName']], extensions: [[$class: 'PruneStaleBranch']], userRemoteConfigs: [[credentialsId: '0a03e51e-969b-41a2-9393-630055229bfc', url: 'http://gitlab.eegrid.com/nxt-pc/efos3.0.git']]])
    ...
    stage('同步静态资源'){
        echo IP
        withEnv(["IP=$IP"]) {
            sh 'rsync -avrz src/platform/$platform/static/_@server_resource/ root@${IP}:/var/www/oms_huangjian/web_$platform/_@server_resource/'
        }
    }
    stage('构建打包') {
            ...
            publishers: [
                sshPublisherDesc(
                    ...
                    transfers: [
                        ...
                        '''#! /bin/bash
                            shopt -s extglob 
                            cd /var/www/oms_huangjian/web_$platform 
